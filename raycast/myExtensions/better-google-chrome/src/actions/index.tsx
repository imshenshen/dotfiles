...
/*  await runAppleScript(`
  ...
  await runAppleScript(`
    ...
    -- Get the max id
    set maxID to do shell script "USER=$(whoami) /opt/homebrew/bin/yabai -m query --windows --space | /opt/homebrew/bin/jq 'max_by(.id) | .id'"
    
    -- Toggle float on the window with max id and set it to center of the screen
    do shell script "USER=$(whoami) /opt/homebrew/bin/yabai -m window " & maxID & " --toggle float --grid 10:4:1:1:2:8"
    return true
  `);
}
